...
public class Player {
    ...
    public SeaBoard.MissileResult placeMissile(SeaBoard targetBoard) {
        ...
        int[] shotCoord = Utils.parseStringCoordinate(shot);
    ...
    public static ValidationResult isValidShipCoordinates(String startCoordinate, String endCoordinate, Ship ship) {
        if (!Utils.isValidCoordinate(startCoordinate) || !Utils.isValidCoordinate(endCoordinate)) {
            return new ValidationResult(false, "Error! Wrong ship location! Try again:");
        }
        int[] start = Utils.parseStringCoordinate(startCoordinate);
        int[] end = Utils.parseStringCoordinate(endCoordinate);

        if (start[0] != end[0] && start[1] != end[1]) {
            return new ValidationResult(false, "Error! Wrong ship location! Try again:");
        }
        if (ship.getCells() != Utils.calculateLength(start, end)) {
            String errorString = String.format("Error! Wrong length of the %s! Try again:", ship.getName());
        ...
        for (Ship ship : shipsToPlace) {
            ...
            while (!coordResult.isValid || !placeResult.isValid) {
                ...
                if (coordResult.isValid) {
                    int[] startInt = Utils.parseStringCoordinate(startCoordinate);
                    int[] endInt = Utils.parseStringCoordinate(endCoordinate);
